...
from src.domain.interfaces.refresh_token_repository import RefreshTokenRepository as RefreshTokenRepo
...
class RefreshTokenRepository(RefreshTokenRepo):
    ...
    async def get_refresh_token_by_jti(self, jti: str) -> RefreshToken | None:
        result = await self.session.execute(select(RefreshToken).filter(RefreshToken.jti == jti))
